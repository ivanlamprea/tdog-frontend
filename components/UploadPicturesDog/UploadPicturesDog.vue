<template>
    <div class="card">
        <Toast />
        <FileUpload name="files[]" :fileLimit="5" @upload="onTemplatedUpload($event)" :customUpload="true" :multiple="true" accept="image/*" :maxFileSize="1000000" @select="onSelectedFiles">
            <template #header="{ chooseCallback, uploadCallback, clearCallback, files }">
                <div class="flex flex-wrap justify-between items-center flex-1 gap-2">
                    ...
                    <ProgressBar :value="totalSizePercent" :showValue="false" :class="['md:w-[20rem] h-4 w-full md:ml-auto', { 'exceeded-progress-bar': totalSizePercent > 100 }]"><span class="whitespace-nowrap">{{ totalSize }}B / 1Mb</span></ProgressBar>
                </div>
            </template>
...
<script setup>
    ...
    const { uploadPictures } = useDogStore();
    const { uploaded_pictures } = storeToRefs(useDogStore());
    ...
    const uploadEvent = async (callback) => {
        ...
        for(let i = 0; i < files.value.length; i++ ) {
            formData.append('pictures', files.value[i]);
        }
        await uploadPictures(dogId,formData);
        totalSizePercent.value = totalSize.value / 10;
        callback();
        navigateTo('/pets')
    };
